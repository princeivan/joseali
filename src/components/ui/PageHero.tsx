...
export default function PageHero({ title, subtitle, imageSeed, imageAlt }: Props) {
  return (
    <section className="relative flex h-[280px] items-end overflow-hidden bg-forest-950 sm:h-[320px]">
      <Image
        src={`https://picsum.photos/seed/${encodeURIComponent(imageSeed)}/1600/500`}
        alt={imageAlt}
        fill
        sizes="100vw"
      ...
      <div className="container-edge relative pb-8">
        ...
        {subtitle && <p className="mt-2 max-w-lg text-sm text-sand-100/85">{subtitle}</p>}
      </div>
    </section>
  );
}
